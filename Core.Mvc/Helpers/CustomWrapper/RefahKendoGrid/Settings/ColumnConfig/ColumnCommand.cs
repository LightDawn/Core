...
{   
    ...
    {
        public GCommandRP? Name { get; set; }
            ...
            if (Name != null)
            {
                switch (Name)
                {
                    ...
                    case GCommandRP.Refresh:
                        json["template"] = string.Format("<a id=\"k_ref_btn_{0}\"  class=\"{1}\"  href=\"\\\\#\" ><span  class=\"{2}\" ></span></a>",
                            GridID, StyleKind.CommandButton, StyleKind.Icons.Refresh);
                        break;
        ...
        {
            return string.Format("<script> $(\"[id='{2}_ns_btn_{3}'] \").on('click' , function() {0}  ns_Search.srch($('\\\\#{2}')) {1}); <\\/script>",
                                "{",
                                "}",
                                 GridID,
                                 "srch");
        ...
        public string GetAddEditScriptEventListener(GCommandRP buttonType)
        {
            ...
            return string.Format("<script> $('\\\\#{0}_ns_btn_{1}').on('click' , function(e) {{  if($(this).attr('disabled')!== 'disabled') {2} }}); <\\/script>",
                GridID,
                buttonType == GCommandRP.Create ? "ad" : "ed",
                buttonType == GCommandRP.Create ? addListener : editListener);
        }

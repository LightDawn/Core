...
namespace Core.Mvc.Helpers.CustomWrapper.DataSource
{
    ...
    public abstract class DataSourceBase : JsonObjectBase
    {
        ...
        {
           // var dataTableEnumerable = RawData as DataTableWrapperRP;
        ...
        {
            if (!string.IsNullOrEmpty(dataSourceInfo.ModelRP.ModelIdName))
            {   //At this point,we know that the Schema has been filled with our desired values,otherwise we should have transfered an instance of DataSourceBase as a parameter.
                this.Schema.Model.Id = new DataKeyRP(dataSourceInfo.ModelRP.ModelIdName);
            }
            else
            {
                this.Schema.Model.Id = new DataKeyRP("Id");
            }
        ...
        public void Process(DataSourceRequest request, bool processData)
        {
            ...
            if (Data != null)
            {
                ...
                {
                    var wrapper = Data as IGridCustomGroupingWrapperRP;
                    if (wrapper != null)
                    {
        ...
        #region Events
            public Dictionary<DataSourceEvent, object> AssignDsEvents(Dictionary<DataSourceEvent, object> dictionary)
            {
                if (dictionary.Any(k => k.Value.ToString() != string.Empty))
                {
                    ...
                    if (tobeAppendToCustomHandlers.Count > 0)
                    {
                        ...
                        dictionary = MakeDataSourceEvents(refinedDic);
                    }
                ...
                {
                    dictionary = MakeDataSourceEvents(dictionary);
                }
                return dictionary;
            }

            private Dictionary<DataSourceEvent, object> MakeDataSourceEvents(Dictionary<DataSourceEvent, object> recivedEvents)
            {

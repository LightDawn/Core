...
using Core.Cmn.FarsiUtils.Exceptions;
...
namespace Core.Cmn.FarsiUtils
{
    ...
    /// </summary>
    [TypeConverter("Core.Cmn.Win.Design.PersianDateTypeConverter")]
    [Serializable]
        ...
        IEquatable<PersianDate>
    {
        ...
        public static bool IsValidYear(int YearNo)
        {
            return !(YearNo < 1 || YearNo > 9378);
        }
        ...
        public static bool IsValidMonth(int MonthNo)
        {
            return !(MonthNo > 12 || MonthNo < 1);
        }

        private static void CheckDay(int YearNo, int MonthNo, int DayNo)
        {
            if (!IsValidDay(YearNo, MonthNo, DayNo))
        ...
        public static bool IsValidDay(int YearNo, int MonthNo, int DayNo)
        {
            ...
            {
                if (!pc.IsLeapYear(YearNo) || DayNo > 30)
                    return false;
            }
            return true;
        }
        ...
        public  static List<int> GetYears(int sinceDuration,int untilDuration)
        {
            var list = new List<int>();
            var  dt=PersianDateConverter.ToPersianDate(DateTime.Now);
            for (int i = dt.Year - sinceDuration; i < dt.Year+untilDuration; i++)
                list.Add(i);
            return list;
        }
        #endregion
        ...
        public static bool IsValidPersianDate(string persianDateStirng)
        {
            int year, month, day;
            var splitedStringDate = persianDateStirng.Split('/');
            if (splitedStringDate.Length != 3 ||
                !int.TryParse(splitedStringDate[0], out year) ||
                !int.TryParse(splitedStringDate[1], out month) ||
                !int.TryParse(splitedStringDate[2], out day))
                return false;
            if (!IsValidDay(year, month, day) || !IsValidMonth(month) || !IsValidYear(year))
                return false;

            //if (!(year > 0 && year < 9379)) return false;
            //bool isLeapYear = pc.IsLeapYear(year);
            //if (month > 0 && day > 0 &&
            //    ((month <= 6 && day <= 31) ||
            //    (month > 6 && month < 12 && day <= 30) ||
            //    (month == 12 && !isLeapYear && day <= 29) ||
            //    (month == 12 && isLeapYear && day <= 30)))
            //    return true;
            //else 
            return true;
        ...
        public string ToString(string format, IFormatProvider formatProvider)
        {
            ...
            switch (format)
            {
                ...
                case "DD":
                    //'yyyy mm dd dddd' e.g. 'دوشنبه بیست شهریور هزار و سیصد و هشتاد و چهار'
                    {
                      
                        switch (Day)
                        {
                            case 1:
                                return string.Format("{0} {1} {2} {3}", new object[4] { LocalizedWeekDayName, "اول", LocalizedMonthName, ToWords.ToString(Year) });
                                
                            case 3:
                                return string.Format("{0} {1} {2} {3}", new object[4] { LocalizedWeekDayName, "سوم", LocalizedMonthName, ToWords.ToString(Year) });
                                
                            default: return string.Format("{0} {1} {2} {3}", new object[4] { LocalizedWeekDayName, ToWords.ToString(int.Parse(Util.toDouble(Day))) + "م", LocalizedMonthName, ToWords.ToString(Year) });
                        }
